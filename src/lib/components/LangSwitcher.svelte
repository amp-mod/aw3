...
<Select.Root type="single" value={getLocale()} onValueChange={(v) => v && setLocale(v as any)}>
	<Select.Trigger
		class="flex h-10 min-w-64 items-center justify-between gap-2 rounded-md border border-neutral-200 bg-white px-4 py-2 text-sm font-medium text-neutral-900 cursor-pointer focus-visible:ring-2 focus-visible:ring-accent focus-visible:outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-white"
	>
	...
	<Select.Content
		sideOffset={8}
		class="z-50 max-h-96 min-w-64 overflow-auto rounded border border-neutral-300 bg-white py-1 outline-none dark:border-neutral-700 dark:bg-neutral-800"
	>
